...
gcloud compute ssh --project=$ID --zone=$ZONE $INSTANCE_NAME -- 'sudo apt-get -y update && sudo apt-get install -y git python3-setuptools python3-dev build-essential && curl https://bootstrap.pypa.io/get-pip.py -o get-pip.py && sudo python3 get-pip.py && git clone https://github.com/GoogleCloudPlatform/training-data-analyst && cd ~/training-data-analyst/courses/developingapps/python/devenv/ && sudo python3 server.py'
